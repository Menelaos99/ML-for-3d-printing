...
#%%
IoU , gpbo_parameters = parameter_extraction_function()
gpbo_parameters  = abs(gpbo_parameters)
print(IoU.shape)
if inversion_flag :
    IoU = 1/IoU
# print(IoU)
...
class GPRegressionModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            # self.covar_module = gpytorch.kernels.SpectralMixtureKernel(num_mixtures=4, ard_num_dims=5) + gpytorch.kernels.LinearKernel()
...
def get_score(Xsamples,optimal_so_far, model,method = "pe",tradeoff = 0.5 ):
    likelihood.eval()
    model.eval()
    mu, std = model(Xsamples).mean.detach().numpy(), np.sqrt(model(Xsamples).variance.detach().numpy())
...
def get_suggestions(optimal_so_far, model,method = "pe",humidity_value = 21,temperature_value = 25, tradeoff = 0.5):
    ...
    if normalization_flag:
        Xsamples[:,0] = normalization_with_inputs(humidity_value, param_max[0],param_min[0])
        Xsamples[:,1] = normalization_with_inputs(temperature_value, param_max[1],param_min[1])
        Xsamples[:,2] = normalization_with_inputs(Xsamples[:,2]*950 + 50, param_max[3],param_min[3])
        Xsamples[:,3] = normalization_with_inputs(Xsamples[:,3]*600+200, param_max[2],param_min[2])
        Xsamples[:,4] = normalization_with_inputs(Xsamples[:,4]*2+3.5, param_max[4],param_min[4])
    else:
        Xsamples[:,0] = humidity_value
        Xsamples[:,1] = temperature_value
        Xsamples[:,2] = Xsamples[:,2]*950 + 50
        Xsamples[:,3] = Xsamples[:,3]*600+200
...
for tradeoff in [0.2]:
    for method in ["pe"]:
        ...
        print(' humidity,        temp,    divisor,        speed,       hv')
        print(best[0])
        print()
# %%
# scores = get_score(Xsamples,optimal_so_far, model,method, tradeoff)
